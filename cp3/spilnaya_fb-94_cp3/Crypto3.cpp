...
void Bigram()
{
    ...
    FILE* plain;
    ofstream fout;
    fout.open("D:\\Crypto3\\bigram.txt");
    fopen_s(&plain, "D:\\Crypto3\\06.txt", "r");
        ...
        for (size_t i = 0; i < setlen; i++) {
            for (size_t j = 0; j < setlen; j++) {
                ...
                if (n > 0) {
                    ...
                    fr = n / k;
                    entrop = fr * log2(fr);
                    all_entr = all_entr + entrop;
                }
            }
        }
    }
    double ans_entr = (all_entr / 2) * -1;
    cout << "Ентропия : " << ans_entr << endl;
}
void Find_Key()
{
    int a, b, c1 = 0, c2 = 0;
...
void Affine_Сipher()
{
    const char alf[] = "абвгдежзийклмнопрстуфхцчшщыьэюя";
