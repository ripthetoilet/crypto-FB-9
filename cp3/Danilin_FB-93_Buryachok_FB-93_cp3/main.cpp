...
bool check_key(const string& text)
{
	string baned_bigrams[] = { "аь", "уь", "оь", "еь", "иь", "ыь", "эь", "юь", "яь", "йь", "кь", "хь", "ць", "ьа", "ьй", "ьу", "ьы", "ьл", "ьь", "йй", "шш", "щщ", "ыы", "ээ" };
	
	for (int i = 0; i < 24; i++)
	{
		if (text.find(baned_bigrams[i]))
		{
			return false;
		}
	}
...
string uncipher(const string& cipher, int key_a, int key_b)
{
	string text;
...
int main()
{
	...
	fout << uncipher(cipher, key.first, key.second);
