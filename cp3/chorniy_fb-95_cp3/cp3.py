...
combos = lambda lst1, lst2: [(i, j) for j in [(i, j) for j in lst2 for i in lst1] for i in [(i, j) for j in lst2 for i in lst1]] 
getkey = lambda pair: list(set([(i, (bisval(pair[0][1]) - i * bisval(pair[0][0])) % SQ) for i in solver((bisval(pair[0][1]) - bisval(pair[1][1])) % SQ, (bisval(pair[0][0]) - bisval(pair[1][0])) % SQ, SQ)]))
