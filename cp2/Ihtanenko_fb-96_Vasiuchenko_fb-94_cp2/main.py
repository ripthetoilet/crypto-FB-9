...
for i in range(0, len(cipher)):
    decipher.append(decode(cipher[i],keys[i],Alphabet_dict,Alphabet))
...
bl = []
bl = block(plaintext, 14)
for i in lst:
    c = dict(collections.Counter(bl[i]))
    f = {k: c[k] / len(plaintext) for k in c}
    import pandas as pd
    data = pd.DataFrame.from_dict(f,'index').stack().reset_index(level=0)  
    data = data.sort_values(by=0, ascending=0)
    data = data.rename(columns = {'level_0': 'b', 0: 'freq'})
    data = data.reset_index(inplace=False).drop(columns= ['index'])
    most = list(data['b'].head())
