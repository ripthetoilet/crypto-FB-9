...
with open("raw_text.txt", "r", encoding="utf-8") as reader:
    ...
    #частота биграм учитывая пробелы
    def bigram_freq_spaces(): 
        bigram_freq1 = {}
        all_bigrams1 = 0
        h2_sp = 0
        res = {}
        ...
        with open('H_and_R.txt', 'a') as f:
            f.write('With spaces: ' + "\n" + "   H2=" + str(h2_sp/2) + "\n" + "   R=" + str(a3) + "\n" + "--------------" + "\n")
        with open('Bigrams_spaces.csv', 'w', newline="") as output:
            writer = csv.writer(output, delimiter=';')
            writer.writerow(["Біграма", "Частота"])
            for key, value in res.items():
                writer.writerow([key, value])    

    #частота биграм БЕЗ пробелов
    def bigram_freq_nospaces(): 
        bigram_freq2 = {}
        all_bigrams2 = 0
        h2_no_sp = 0 
        res = {}
        ...
        with open('H_and_R.txt', 'a') as f:
            f.write('No spaces: ' + "\n" + "   H2=" + str(h2_no_sp/2) + "\n" + "   R=" + str(a4) + "\n" + "--------------" + "\n")
        with open('Bigrams_no_spaces.csv', 'w', newline="") as output:
            writer = csv.writer(output, delimiter=';')
            writer.writerow(["Біграма", "Частота"])
            for key, value in res.items():
                writer.writerow([key, value])
